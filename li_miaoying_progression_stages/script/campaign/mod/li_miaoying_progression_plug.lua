...
local function give_birth(bred_tier, choice)
    ...
    cm:add_unit_to_faction_mercenary_pool(character:faction(), birthed_unit[1], "renown", birthed_num, 20, birthed_num, 1, "", "", "", true, birthed_unit[2]);
...
local function get_bred_callback(context)
    ...
    li_miao:log("Miao attacker " .. tostring(miao_attacker) .. " defender " .. tostring(miao_defender) .. " fought " .. opponent_faction_name);
    ...
    local times_serviced = pb:get_how_many_times_ability_has_been_used_in_battle(faction_cqi, provide_service_ability_name);
...
local function persistent_callback_factory()
...
local function broadcast_self()
    ...
    li_miao:stage_register("plug", this_stage, progression_callback, stage_enter_callback)
    li_miao:stage_register("plug_preg", this_stage + 1, nil, stage_enter_callback); -- nil callback so you can't naturally advance to it
    li_miao:persistent_initialization_register(this_stage, persistent_callback_factory, "birth checks");
end
